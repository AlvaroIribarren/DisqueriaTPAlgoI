...
void ingreso_tracks(strEstructura vector[], int i)
{
	...
	{
		printf("\n El numero ingresado no cumple con las condiciones. Por favor, ingrese el numero de tracks que tendra el disco numero %d, recuerde que el maximo de tracks es de 25: ", i+1);
		scanf("%d", &vector[i].tracks);	
	}
	return;
}
...
int ingreso_busqueda_binaria(strEstructura vector[], int largo_vector)
{
	...
	printf("Si desea informacion de un CD en especial introduzca 1, a continuacion se le mostraran los datos del mismo.");
	printf("Si no desea ver ningun tipo de informacion, introduzca un numero distinto de 1: ");
	scanf("%i", &num);
...
{	int largoVector, posMaximo, posMinimo, precioPromedio, i;
	...
	largoVector=carga(vector); // funcion que realiza la carga y devuelve el largo del vector para usarlo despues.
	
	system("pause");
	
	precioPromedio= precio_promedio(vector, largoVector); // Calcula el precio promedio en base al vector y su largo y lo devuelve.
	...
	posMaximo= pos_maximo(vector, largoVector); // Halla la posicion del máximo.
	...
	posMinimo= pos_minimo(vector, largoVector); //Halla la posicion del mínimo.
	...
	ordenar_vector (vector, largoVector); //Funcion que unicamente ordena al vector en base a su codigo.
	
	system("pause");
	
	printf("========================================================================");
	
 	mostrar_datos_vector_ordenado(vector, largoVector); // muestra los datos del vector ordenado.
	...
	ingreso_busqueda_binaria(vector, largoVector);  
 	/* funcion que pregunta al usuario si desea buscar algun disco por su codigo y en caso afirmativo muestra todos los
	datos del mismo*/
	
	printf("========================================================================\n");
	
	cantidad_tracks (vector, largoVector); //imprime la cantidad de tracks.
